...
//calibration module added by me not in original
void MPU6050::calibrate(){
    float temp_x = 0;
    float temp_y = 0;
    float temp_z = 0;
    for(int i = 0; i < 6; i++){
        temp_x += getAcceleroRawX();
        temp_y += getAcceleroRawY();
        temp_z += getAcceleroRawZ();
    }
    error_x = temp_x/3; //average reading from temp x ( should be zero)
    error_y = temp_y/3; //average reading form temp y (should be zero)
    error_z = (temp_z/3)-9.8; //(average reading from temp z minus gravity)
}
...
void MPU6050::getOffset(float *accOffset, float *gyroOffset, int sampleSize){
    ...
    for (int i = 0; i < sampleSize; i++){
        ...
        thread_sleep_for(0.01);    //wait between each reading for accuracy 
    } 
}
...
void MPU6050::reset(void){
    ...
    char temp;
    temp = 0x07;
    this->write(MPU6050_ADDRESS,temp);
}

void MPU6050::motion_interrupt(void){
    ...
    temp = 0x20; //interupt active high, push-pull signal, stays until register is read
    this->write(MPU6050_INT_PIN_CFG, temp);

    //turn on digital high pass filter, helps isolate rapid change (e.g a fall)
    temp = this->read(MPU6050_ACCELERO_CONFIG_REG);
    temp |=  0x01; //lets start with 5 hz for the filter 
    this->write(MPU6050_ACCELERO_CONFIG_REG,temp);


    //set motion detect threshold
    temp = MOTION_THRESHOLD;
    this->write(MPU6050_MOTION_DETECT_REG,temp);

    //set motion duration
    temp = 40; //set to 40 ms
    this->write(MPU6050_MOTION_DUR_REG, temp);

    //motion detection control
    temp = 0x15; //free-fall and motion decrements to 1 and accel start up delay to 5ms
    this->write(MPU6050_MOTION_CNTRL_REG,temp);

    //enable motion detected Interrupt
    temp = 0x40;
    this->write(MPU6050_RA_INT_ENABLE, temp);
